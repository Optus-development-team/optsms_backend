...
- **AI/Agents:** Google ADK (Generative AI) - *Beta Typescript implementation*
...
* **Connection Pooling (CRITICAL):**
    ...
    * *Why:* To prevent exhausting the database connection limit in high-concurrency scenarios (multiple WhatsApp webhooks).
    * *Connection String Mode:* `pgbouncer=true` or `?pgbouncer=true`.
...
3.  **Resolution Logic:**
    ...
    * *Exception:* If Phone ID is unknown, ignore request (Security by Obscurity).
...
* **Persistence:** The ADK Session state is stored in the `adk_sessions` table (JSONB `context_data`).
    * *Load:* On inbound message, fetch JSONB from DB -> Hydrate ADK Session.
    * *Save:* After agent reply, serialize ADK Session -> Update DB.
...
2.  **Action:** Agent generates a Google OAuth2 authorization URL via `google-auth-library`.
    ...
    * *Access Type:* `offline` (Mandatory for Refresh Token).
    * *State Param:* Encoded JSON `{ company_id: "...", admin_phone: "..." }` to track origin.
    * *Redirect URI:* `https://api.optusbms.com/auth/google/callback`.
...
The Agent must instruct the Payment Backend *which* company's bank credentials to use by passing the `company_id`.
...
* **Logic:**
    ...
    2.  Find the `ROLE_ADMIN` phone number for *that specific Company* in Supabase.
...
## 11. Error Handling
* **Payment Timeout:** If `VERIFICATION_RESULT` takes > 60s, Agent must inform user: *"El banco demora un poco. Te avisaré apenas confirme."* and schedule a delayed check.
