...
export class MetaCatalogService implements OnModuleInit {
  ...
  constructor(
    ...
    this.syncOnStartup = this.configService.get<string>(
      'CATALOG_SYNC_ON_STARTUP',
      'true',
    ) === 'true';
  }
  ...
  async onModuleInit(): Promise<void> {
    if (!this.syncOnStartup) {
      this.logger.log('Sincronización de catálogo al inicio deshabilitada (CATALOG_SYNC_ON_STARTUP=false)');
      return;
    }

    if (!this.apiToken) {
      this.logger.warn('META_API_TOKEN no configurado. Sincronización de catálogo omitida.');
      return;
    }

    if (!this.supabase.isEnabled()) {
      this.logger.warn('Supabase no habilitado. Sincronización de catálogo omitida.');
      return;
    }

    this.logger.log('Iniciando sincronización de catálogos de Meta al iniciar el backend...');
   ...
   */
  async syncAllCatalogs(): Promise<{ total: number; synced: number; errors: number }> {
    try {
      ...
      this.logger.log(`Sincronizando catálogos de ${companies.length} compañías...`);
